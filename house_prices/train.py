...
def _persist_artifacts(model, pp: Dict[str, object], num_cols: List[str], cat_cols: List[str], feature_order: List[str]) -> None:
...
def build_model(data: pd.DataFrame) -> Dict[str, float]:
    ...
    pp = fit_preprocessors(X_tr, num_cols, cat_cols, create_preprocessors())
    ...
    _persist_artifacts(model, pp, num_cols, cat_cols, Xtr_proc.columns.tolist())
    return {"rmsle": rmsle}
