...
def fit_preprocessors(X_train: pd.DataFrame, num_cols: List[str], cat_cols: List[str], pp: Dict[str, object]) -> Dict[str, object]:
...
def transform_features(X: pd.DataFrame, num_cols: List[str], cat_cols: List[str], pp: Dict[str, object]) -> pd.DataFrame:
    ...
    cat_names = pp["ohe"].get_feature_names_out(cat_cols).tolist()
    return pd.DataFrame(
        np.hstack([Xn, Xc]),
